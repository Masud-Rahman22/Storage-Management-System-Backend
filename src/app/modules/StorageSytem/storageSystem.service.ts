...
import { sendImageToCloudinary } from '../../middlewares/uploadFiles';
...
const shareFolder = async (folderID: Types.ObjectId, userEmail: string) => {
  // Find the user
...
const deleteFolder = async (folderID: string) => {
  await FolderModel.findByIdAndUpdate(folderID, { isDeleted: true });
...
const createFile = async (
  ...
  const user = await User.findOne({ email: currentUserEmail }).select(
    '_id limit',
  );
  if (!user) {
    throw new AppError(httpStatus.NOT_FOUND, 'User not found');
...
console.log('1');
const { secure_url, public_id } = await sendImageToCloudinary(file.originalname, file.buffer);
...
const shareFile = async (fileID: Types.ObjectId, userEmail: string) => {
  // Find the user
  const user = await User.findOne({ email: userEmail }).select('_id');
...
const deleteFile = async (fileID: string) => {
  await FileModel.findByIdAndUpdate(fileID, { isDeleted: true });
...
const getData = async (
  ...
  if (dataType) {
    if (
      ...
      throw new AppError(
        ...
        `type must be string and value will be 'PDF', 'Image', 'Document'`,
      );
    }
    const files = await FileModel.find({
      userID: userData._id,
  ...
  if (day) {
    ...
    const endOfDay = new Date(
      Date.UTC(year, month - 1, dayOfMonth, 23, 59, 59),
    );
    const folders = await FolderModel.find({
      userID: userData._id,
  ...
  if (fileID) {
    ...
    if (fileData.isSecured) {
      ...
      if (!secureFolderToken) {
        throw new AppError(
          httpStatus.UNAUTHORIZED,
          'Unauthorized to access secure file',
        );
      }
