...
const googleAuth = async (tokenId: string) => {
  ...
  const result: any = await gClient.verifyIdToken({
    idToken: tokenId,
  ...
  if (user) {
    ...
    try {
      ...
      const newRootFolder = await FolderModel.create([RootfolderData], {
        session,
      });
      ...
      if (!updateResult.matchedCount || !updateResult.modifiedCount) {
        throw new AppError(
          httpStatus.BAD_REQUEST,
          'Failed to associate root folder with user',
        );
      }
...
const changePassword = async (
  ...
  if (
    user.password &&
    !(await User.isPasswordMatched(payload.oldPassword, user?.password))
  ) {
...
const forgetPassword = async (email: string) => {
  ...
  if (OTPExpiresAt && currentTime < OTPExpiresAt) {
    throw new AppError(httpStatus.FORBIDDEN, 'OTP already generated recently');
  }
...
const verifyOTP = async (email: string, OTP: string) => {
  ...
  if (
    ...
    throw new AppError(httpStatus.FORBIDDEN, 'OTP verification failed');
  }
...
const resetPassword = async (token: string, password: string) => {
  ...
  if (!updateResult.matchedCount || !updateResult.modifiedCount) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      'Failed to remove verification information',
    );
  }
...
const createPinForSecureFolder = async (userData: JwtPayload, PIN: string) => {
  ...
  try {
    ...
    const securedRootFolder = await FolderModel.create([securefolderData], {
      session,
    });
    ...
    if (!securedRootFolder.length) {
      throw new AppError(
        httpStatus.BAD_REQUEST,
        'Failed to create secured Root folder',
      );
    }

    const HashedPin = await bcrypt.hash(PIN, Number(config.bcrypt_salt_round));
    //Update the user with rootFolderID
    const updateResult = await User.updateOne(
      { _id: user._id },
      {
...
const LoginToSecureFolder = async (userData: JwtPayload, PIN: string) => {
  ...
  if (!user.secureFolderPin || !user.securedrootFolderID) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      'Pin and secure folder are not created',
    );
  }
  ...
  if (
    user?.password &&
    !(await User.isPasswordMatched(PIN, user.secureFolderPin))
  ) {
    throw new AppError(httpStatus.FORBIDDEN, 'Password is not correct');
  }
...
const getMyProfile = async (email: string) => {
  ...
  const storageUsageByType = sizesByType.reduce(
    (acc, { _id, totalSize, totalItems }) => {
  ...
  const recentFolders = await FolderModel.find({
    userID: user._id,
    isSecured: false,
  })
    .sort({ updatedAt: -1 })
