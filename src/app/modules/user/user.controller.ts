...
const signUpUserByEmailandPassword: RequestHandler = catchAsync(
  async (req, res) => {
    const { refreshToken, accessToken, user } = await UserServices.SignUp(
      req.body,
    );
...
const LoginUserByEmailandPassword: RequestHandler = catchAsync(
  async (req, res) => {
...
const LoginwithGoogle: RequestHandler = catchAsync(async (req, res) => {
  const { user, accessToken, refreshToken } = await UserServices.googleAuth(
    req.body?.tokenId,
  );
...
const LoginToSecureFolder = catchAsync(async (req, res) => {
  ...
  const { securedrootFolderID, accessToken } =
    await UserServices.LoginToSecureFolder(req.user, PIN);
